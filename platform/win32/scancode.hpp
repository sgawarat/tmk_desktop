...
namespace /* TMK_DESKTOP_WIN32_ALL_SCANCODES */ {
#define TMK_DESKTOP_WIN32_ALL_SCANCODES(m) \
    m(ESCAPE, 0x0001) \
    m(1, 0x0002) \
    m(2, 0x0003) \
    m(3, 0x0004) \
...
#define TMK_DESKTOP_WIN32_SCANCODE_ALIASES(m) \
    m(F24, 0x0076) \
    m(POWER, 0xe05e)
}  // namespace
...
/**
 * @brief キーコードからスキャンコードに変換する
 */
uint16_t make_scancode(Keycode keycode) noexcept;

/**
 * @brief スキャンコードを示す文字列を取得する
 */
